...
import { useAddNewsMembers } from '../hooks/useNewsMembers';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { useParams } from 'react-router';
import { BTK_GET_NEWS_MEMBERS } from '~/modules/news/graphql/newsQueries';

export const NewsDetailAddMember = ({ members }: { members: string[] }) => {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <Sheet.View>
        ...
        {open && (
          <NewsDetailAddMemberForm setOpen={setOpen} members={members} />
        )}
      </Sheet.View>
    </Sheet>
  );
...
export const NewsDetailAddMemberForm = ({
  setOpen,
  members,
}: {
  ...
  const { addNewsMembers, loading } = useAddNewsMembers();
  ...
  const onSubmit = (data: { memberIds: string[] }) => {
    ...
    addNewsMembers({
      variables: { news: id, memberIds: newMemberIds },
      onCompleted: () => {
