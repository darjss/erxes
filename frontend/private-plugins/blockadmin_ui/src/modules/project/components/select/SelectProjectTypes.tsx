...
const SelectProjectTypesValue = ({ placeholder }: { placeholder?: string }) => {
  const { values } = useSelectProjectTypesContext();
  ...
  const selectedTypes = values.map(
    (value) => PROJECT_TYPES.find((type) => type.value === value)?.label,
  );
...
export const SelectProjectTypes = Object.assign(SelectProjectTypesRoot, {
