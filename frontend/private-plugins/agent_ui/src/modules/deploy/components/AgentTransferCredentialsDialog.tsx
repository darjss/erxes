...
import { CREATE_AGENT_TRANSFER_CREDENTIALS } from '../graphql/mutations';

interface AgentTransferCredentialsDialogProps {
  open: boolean;
...
export const AgentTransferCredentialsDialog = ({
  open,
  onOpenChange,
}: AgentTransferCredentialsDialogProps) => {
  const identifierId = useCurrentIdentifierId();
  const [loadCredentials, { data, loading, error }] = useMutation(
    CREATE_AGENT_TRANSFER_CREDENTIALS,
  );
  const credentials = data?.createAgentTransferCredentials;
        ...
        <AlertDialog.Header className="flex flex-row gap-3 sm:flex-row">
          ...
          <div className="flex flex-col gap-2 text-left">
            <AlertDialog.Title className="text-base font-semibold">
              Assistant transfer credentials
            </AlertDialog.Title>
            <AlertDialog.Description className="text-muted-foreground text-sm">
              Paste these values into another SaaS to link this same assistant server there.
