...
import {
  BTK_ADD_PROJECT_MEMBERS,
  BTK_DELETE_PROJECT_MEMBER,
  BTK_UPDATE_PROJECT_MEMBER,
} from '../graphql/projectMutations';
import { toast } from 'erxes-ui';
import { BTK_GET_PROJECT_MEMBERS } from '@/project/graphql/projectQueries';
...
export const useUpdateProjectMember = () => {
  ...
  const handleUpdateProjectMember = ({
    ...
    updateProjectMember({
      ...
      update: (cache, { data }) => {
        cache.modify({
          id: cache.identify(data.btkUpdateProjectMember),
          fields: {
            role: () => role,
          },
          optimistic: true,
...
export const useDeleteProjectMember = () => {
  ...
  const [deleteProjectMember, { loading, error }] = useMutation(
    BTK_DELETE_PROJECT_MEMBER,
    {
      ...
      refetchQueries: [
        { query: BTK_GET_PROJECT_MEMBERS, variables: { project: id } },
      ],
    },
  );
