...
export const validateProviderOwnership = async (
  ...
  try {
    const result = await context.masterClient.query<{
      oneFitProvider: { _id: string; instanceId?: string } | null;
    }>(query, { id: providerId });

    const provider = result.oneFitProvider;
...
export const validateProviderOwnershipByActivityType = async (
  ...
  try {
    ...
    const activityType = result.oneFitActivityType;
