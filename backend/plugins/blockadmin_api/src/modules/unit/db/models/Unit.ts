...
export interface IUnitModel extends Model<IUnitDocument> {
  ...
  updateUnit(subdomain: string, entityId: string, input: IUnit): Promise<IUnitDocument>;
...
export const loadUnitClass = (models: IModels) => {
  class Unit {
    ...
    public static async updateUnit(subdomain: string, entityId: string, input: IUnit) {
      const unit = await models.Unit.getUnit(subdomain, entityId);
      if (!unit) throw new Error(`Unit "${entityId}" not found in subdomain "${subdomain}"`);
      return models.Unit.findOneAndUpdate({ _id: unit._id }, input, { new: true });
    }
    ...
    public static async transferUnit(payload: ITransferUnit) {
      ...
      if (!unit) {
        throw new Error(`Unit "${unitId}" not found in subdomain "${blockSubdomain}"`);
      }
