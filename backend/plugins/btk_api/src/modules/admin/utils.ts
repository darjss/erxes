...
const { BTK_ADMIN_API_URL, BTK_ADMIN_SECRET } = process.env;
...
const sendMessage = ({ subdomain, path, payload }: SendMessagePayload) => {
  const API_ENDPOINT = `${BTK_ADMIN_API_URL}/api/${path}`;

  if (!BTK_ADMIN_API_URL || !BTK_ADMIN_SECRET) {
    return console.log('BTK_ADMIN_API_URL or BTK_ADMIN_SECRET is not set');
  }
  ...
  try {
    ...
    console.error(`Failed to send message to btk-admin: ${e}`);
  }
};
