...
export const offerMutations = {
  btkCreateOffer: async (
    ...
    if (paymentPlan.frequency === BtkProjectPaymentPlanFrequency.CUSTOM) {
    ...
    if (frequency === BtkProjectPaymentPlanFrequency.ONE_TIME) {
      ...
      for (let i = 0; i < installment; i++) {
        ...
        if (interestPercentage && interestPercentage > 0) {
          switch (interestType) {
            ...
            case BtkProjectPaymentPlanInterestType.FLAT: {
...
requireLogin(offerMutations, 'btkCreateOffer');
requireLogin(offerMutations, 'btkUpdateOffer');
