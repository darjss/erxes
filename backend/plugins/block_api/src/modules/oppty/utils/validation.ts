import { DEFAULT_STATUS_TYPES } from '~/modules/status/constants';
import { IOpptyDocument } from '../@types/oppty';
import { IModels } from '~/connectionResolvers';

type StatusKey = typeof DEFAULT_STATUS_TYPES[keyof typeof DEFAULT_STATUS_TYPES];
...
export const STATUS_VALIDATION_FIELDS = {
    [DEFAULT_STATUS_TYPES.LEAD]: ['name', 'assignedUserId', 'customerId', 'projectId'],
}
...
export const STATUS_VALIDATION = {
  [DEFAULT_STATUS_TYPES.LEAD]: async (
    ...
    if (status === DEFAULT_STATUS_TYPES.LEAD) {
      ...
      const VALIDATION_FIELDS = STATUS_VALIDATION_FIELDS[DEFAULT_STATUS_TYPES.LEAD];
      ...
      if (VALIDATION_FIELDS.length) {
        throw new Error('The oppty is missing required fields to move to Lead status: ' + VALIDATION_FIELDS.join(', '));
      }
    }
  },
